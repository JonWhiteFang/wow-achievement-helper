...
type ChildCriterion = {
  id: number;
  description: string;
  linked_achievement?: { id: number; name: string };
  amount?: number;
};
...
describe("manifest meta achievement detection", () => {
  it("detects meta achievements from linked_achievement in child_criteria", () => {
    const mockAchievementData: MockAchievement = {
      ...
      criteria: {
        child_criteria: [
          { id: 1, description: "The Dedicated Few", linked_achievement: { id: 562, name: "The Dedicated Few" } },
          { id: 2, description: "Arachnophobia", linked_achievement: { id: 1858, name: "Arachnophobia" } },
        ],
      },
    };

    const hasLinkedAchievements = mockAchievementData.criteria?.child_criteria?.some((c) => c.linked_achievement);
  ...
  it("does not mark regular achievements as meta", () => {
    const mockAchievementData: MockAchievement = {
      ...
      criteria: { child_criteria: [{ id: 1, description: "Kill 10 enemies", amount: 10 }] },
    };
  ...
  it("handles mixed criteria (some linked, some not)", () => {
    const mockAchievementData: MockAchievement = {
      ...
      criteria: {
        child_criteria: [
          { id: 1, description: "Sub Achievement", linked_achievement: { id: 400, name: "Sub Achievement" } },
          { id: 2, description: "Regular criterion", amount: 5 },
        ],
      },
    };
